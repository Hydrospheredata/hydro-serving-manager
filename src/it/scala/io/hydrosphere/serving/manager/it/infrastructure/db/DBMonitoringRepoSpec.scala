...
class DBMonitoringRepoSpec extends FullIntegrationSpec with IOChecker {
  ...
  override protected def beforeAll(): Unit = {
    ...
    val f = for {
      ...
      mvOld = ModelVersion.Internal(1, DockerImage("qwe", "asdasd"), Instant.now(), Some(Instant.now()), 1, ModelContract.defaultInstance, dummyImage, m, None, ModelVersionStatus.Released, None, Map.empty, Some(MonitoringConfiguration()))
