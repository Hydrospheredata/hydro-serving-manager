...
class DBModelVersionRepoSpec extends FullIntegrationSpec with IOChecker {
  ...
  describe("Queries") {
    val row = ModelVersionRow(
      ...
      metadata = Some("{}"),
      is_external = false,
      monitoring_configuration = Some(MonitoringConfiguration(batch_size = 10))
    )
  ...
  override def beforeAll(): Unit = {
    ...
    val f = for {
      ...
      version = ModelVersion.Internal(
        ...
        monitoringConfiguration = Some(MonitoringConfiguration(batch_size = 10))
      )
    }
    f.unsafeRunSync()
  }
