...
trait Converter {
  ...
  /**
    ...
    */
  def akkaSourceToFs2Stream[F[_]: Async, A](
      source: Graph[SourceShape[A], NotUsed]
  )(implicit materializer: Materializer): Stream[F, A] =
    Stream.force {
      Async[F].delay {
  ...
  /**
    ...
    */
  def akkaSourceToFs2StreamMat[F[_]: Async, A, M](
      source: Graph[SourceShape[A], M]
  )(implicit materializer: Materializer): F[(Stream[F, A], M)] =
    Async[F].delay {
      val (mat, subscriber) =
  ...
  /**
    ...
    */
  def akkaSinkToFs2Pipe[F[_]: Async, A](
      sink: Graph[SinkShape[A], NotUsed]
  )(implicit materializer: Materializer): Pipe[F, A, Unit] =
    (s: Stream[F, A]) =>
      Stream.force {
        Async[F].delay {
          val publisher =
  ...
  /**
    ...
    * and the materialized value of the [[Graph]].
    */
  def akkaSinkToFs2PipeMat[F[_]: Async, A, M](
      sink: Graph[SinkShape[A], M]
  )(implicit materializer: Materializer): F[(Pipe[F, A, Unit], M)] =
    Async[F].delay {
      val (publisher, mat) =
    ...
    for {
      ...
      fs2Sink <- akkaSinkToFs2PipeMat[F, A, Future[M]](akkaSink).flatMap {
        case (stream, mat) =>
          ...
          val callback = Async[F].delay(mat.onComplete {
            case Failure(ex)    => dispatcher.unsafeRunSync(promise.complete(ex.asLeft))
            case Success(value) => dispatcher.unsafeRunSync(promise.complete(value.asRight))
          })
  ...
  /**
    ...
    * If the materialized value needs be obtained, use [[akkaSinkToFs2PipeMat]].
    */
  def akkaFlowToFs2Pipe[F[_]: Async, A, B](
      flow: Graph[FlowShape[A, B], NotUsed]
  )(implicit materializer: Materializer): Pipe[F, A, B] =
    (s: Stream[F, A]) =>
      Stream.force {
        Async[F].delay {
  ...
  /**
    ...
    * and the materialized value of the [[Graph]].
    */
  def akkaFlowToFs2PipeMat[F[_]: Async, A, B, M](
      flow: Graph[FlowShape[A, B], M]
  )(implicit materializer: Materializer): F[(Pipe[F, A, B], M)] =
    Async[F].delay {
      val src                            = AkkaSource.queue[A](0, OverflowStrategy.backpressure)
      val snk                            = AkkaSink.queue[B]()
  ...
  /**
    ...
    */
  def fs2StreamToAkkaSource[F[_]: Async, A](
      stream: Stream[F, A]
  )(implicit dispatcher: Dispatcher[F]): Graph[SourceShape[A], NotUsed] = {
  ...
  /**
    ...
    * [[Graph]] is materialized.
    */
  def fs2PipeToAkkaSink[F[_]: Async, A](
      sink: Pipe[F, A, Unit]
  )(implicit dispatcher: Dispatcher[F]): Graph[SinkShape[A], Future[Done]] = {
    ...
    val sink2: AkkaSink[SinkQueueWithCancel[A], Future[Done]] = AkkaFlow[SinkQueueWithCancel[A]]
      .map(s =>
        dispatcher.unsafeToFuture(
          subscriberStream[F, A](s).through(sink).compile.drain.as(Done: Done)
        )
      )
      .toMat(AkkaSink.head)(Keep.right)
      .mapMaterializedValue(ffd =>
        dispatcher.unsafeToFuture(Async[F].fromFuture(Async[F].fromFuture(Async[F].pure(ffd))))
      )
  ...
  /**
    ...
    * [[Graph]] is materialized.
    */
  def fs2PipeToAkkaFlow[F[_]: Async, A, B](
      pipe: Pipe[F, A, B]
  )(implicit dispatcher: Dispatcher[F]): Graph[FlowShape[A, B], NotUsed] = {
  ...
  private def subscriberStream[F[_]: Async, A](
  ...
  private def publisherStream[F[_]: Async, A](
      publisher: SourceQueueWithComplete[A],
      stream: Stream[F, A]
  ): Stream[F, Unit] = {
    def publish(a: A): F[Option[Unit]] =
      Async[F]
        .fromFuture(Async[F].delay(publisher.offer(a)))
        .flatMap {
    ...
    def watchCompletion: F[Unit] =
      Async[F].fromFuture(Async[F].delay(publisher.watchCompletion())).void
    ...
    stream
      ...
      .unNoneTerminate
      .onFinalizeCase {
        case Resource.ExitCase.Succeeded | Resource.ExitCase.Canceled => complete
        case Resource.ExitCase.Errored(e)                             => fail(e)
      }
  }
...
trait ConverterDsl extends Converter {
  ...
  implicit class AkkaSourceDsl[A, M](source: Graph[SourceShape[A], M]) {
    ...
    def toStream[F[_]: Async](implicit
    ...
    def toStreamMat[F[_]: Async](implicit
        materializer: Materializer
    ): F[(Stream[F, A], M)] =
    ...
    def toStream[F[_]: Async](
      ...
      Stream.force(akkaSourceToFs2StreamMat(source).map {
        case (akkaStream, mat) =>
  ...
  implicit class AkkaSinkFutureDsl[A, M](sink: Graph[SinkShape[A], Future[M]]) {
    ...
    def toPipeMatWithResult[F[_]: Async: Dispatcher](implicit
        ec: ExecutionContext,
        m: Materializer
    ): F[Pipe[F, A, Either[Throwable, M]]] =
  ...
  implicit class AkkaSinkDsl[A, M](sink: Graph[SinkShape[A], M]) {
    ...
    /** @see [[Converter#akkaSinkToFs2SinkMat]] */
    def toPipeMat[F[_]: Async](implicit
        materializer: Materializer
    ): F[(Pipe[F, A, Unit], M)] =
  ...
  implicit class FS2StreamNothingDsl[A](stream: Stream[Nothing, A]) {
    ...
    def toSource(implicit dispatcher: Dispatcher[IO]): Graph[SourceShape[A], NotUsed] =
  ...
  implicit class FS2StreamIODsl[F[_]: Async: Dispatcher, A](stream: Stream[F, A]) {
  ...
  implicit class FS2SinkIODsl[F[_]: Async: Dispatcher, A](sink: Pipe[F, A, Unit]) {
  ...
  implicit class FS2PipeIODsl[F[_]: Async: Dispatcher, A, B](pipe: Pipe[F, A, B]) {
