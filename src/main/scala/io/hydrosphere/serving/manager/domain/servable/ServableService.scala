...
trait ServableService[F[_]] {
  ...
  ): F[List[GenericServable]]

  def findAndDeploy(
      name: String,
      version: Long,
      metadata: Map[String, String]
  ): F[DeferredResult[F, GenericServable]]
...
object ServableService extends UnsafeLogging {
  def filterByName(name: String) = { (x: List[GenericServable]) => x.filter(_.fullName == name) }
  ...
  def apply[F[_]]()(
    ...
    override def getFiltered(
        ...
        versionId: Option[Long],
        metadata: Map[String, String]
    ): F[List[Servable.GenericServable]] = {
    ...
    override def deploy(
      ...
      for {
        ...
        initServable = Servable(
          modelVersion,
          randomSuffix,
          Servable.Starting("Initialization", None, None),
          Nil,
          metadata
        )
          ...
          .onError {
            case NonFatal(ex) =>
              cloudDriver.remove(initServable.fullName).attempt >>
                d.complete(
                    initServable.copy(status = Servable.NotServing(ex.getMessage, None, None))
                  )
                  .attempt >>
