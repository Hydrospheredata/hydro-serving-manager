...
object KubernetesClient extends UnsafeLogging {
  ...
  def apply[F[_]: Async](
  ...
  )(implicit ex: ExecutionContext): KubernetesClient[F] = new KubernetesClient[F] {
    ...
    override def getLogs(podName: String, follow: Boolean): F[Source[String, _]] = {
      AsyncUtil
        .futureAsync(underlying.getPodLogSource(podName, Pod.LogQueryParams(follow = Some(follow))))
