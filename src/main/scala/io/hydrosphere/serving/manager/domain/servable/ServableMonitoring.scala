...
//TODO: host:port
//TODO: some ? in failed case
object ServableMonitoring extends Logging {
  def make[F[_]](
                  cloudDriver: CloudDriver[F],
                  repo: ServableRepository[F]
                )(implicit F: Concurrent[F]): ServableMonitoring[F] = new ServableMonitoring[F] {
      ...
      cloudDriver.getEvents
        .evalTap { servEvent =>
          val effect = for {
            ...
            instance <- OptionT(cloudDriver.instance(servEvent.instanceName))
    ...
    def getNewStatus(prevStatus: ServableStatus, prevMessage: Option[String], eventType: CloudInstanceEvent): (ServableStatus, Option[String]) = {
      (prevStatus, eventType) match {
        case (ServableStatus.NotAvailable, Available(_)) => (ServableStatus.Serving, none)
        case (ServableStatus.NotAvailable, Ready(_, _)) => (ServableStatus.NotAvailable, prevMessage)
        case (ServableStatus.NotAvailable, NotServing(_, _)) => (ServableStatus.NotAvailable, prevMessage)
        case (ServableStatus.NotServing, NotAvailable(_, _)) => (ServableStatus.NotAvailable, prevMessage)
        case (ServableStatus.NotServing, Available(_)) => (ServableStatus.NotServing, none)
        case (_, Starting(_, warning)) => (ServableStatus.NotServing, warning)
