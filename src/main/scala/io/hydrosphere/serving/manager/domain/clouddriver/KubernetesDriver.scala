...
class KubernetesDriver[F[_]](
  ...
  override def instances: F[List[CloudInstance]] =
    client.services.list.map(_.map(kubeSvc2Servable).collect { case Right(v) => v })
  ...
  override def run(
    ...
    for {
      ...
      newServable <- maybeServable match {
        ...
        case Left(error) =>
          F.raiseError[CloudInstance](
            new RuntimeException(
              s"Cannot create Servable from kube Service $service Reason: $error"
            )
          )
      }
    } yield newServable
...
object KubernetesDriver {
  ...
  def prepareService(name: String, modelVersionId: Long): Service =
    skuber
      .Service(metadata = ObjectMeta(name = name))
      .withClusterIP("None")
