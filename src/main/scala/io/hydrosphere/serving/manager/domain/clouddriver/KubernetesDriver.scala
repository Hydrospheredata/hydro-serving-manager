...
class KubernetesDriver[F[_]: Async](client: KubernetesClient[F]) extends CloudDriver[F] {
  private def kubeSvc2Servable(svc: skuber.Service): Option[CloudInstance] =
    for {
  ...
  override def instances: F[List[CloudInstance]] =
    client.services.map(_.map(kubeSvc2Servable).collect { case Some(v) => v })

  override def instance(name: String): F[Option[CloudInstance]] =
    instances.map(_.find(_.name == name))
  ...
  override def run(
    ...
    for {
      ...
      newServable <- maybeServable match {
        ...
        case None =>
          Async[F].raiseError[CloudInstance](
            new RuntimeException(s"Cannot create Servable from kube Service $service")
          )
      }
    } yield newServable
  }
  ...
  override def remove(name: String): F[Unit] =
    client.removeService(name) *> client.removeDeployment(name)
