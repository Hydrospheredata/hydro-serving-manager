...
object ApplicationMonitoring extends Logging {
  def make[F[_]](
                ...
                )(implicit F: Concurrent[F]): ApplicationMonitoring[F] = new ApplicationMonitoring[F] {
    ...
    def updateApp(event: DiscoveryEvent[Servable, String]): F[Unit] = {
      event match {
        ...
        case DiscoveryEvent.ItemRemove(servables) =>
          servables.traverse { servable =>
            appRepo.findServableUsage(servableName = servable).flatMap { apps =>
              apps.traverse { app =>
                appRepo.update(app.copy(status = Application.Status.Failed, statusMessage = "Servable from graph was deleted".some))
              }
            }
          }.as(())
      }
