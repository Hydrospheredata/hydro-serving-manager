...
object MonitoringService extends UnsafeLogging {

  final val MetricSpecIdKey = "metric-spec-id"

  case class NoMonitoringModelFound(spec: CustomModelMetricSpec)
      extends RuntimeException(
        s"Can't find a model version with id ${spec.modelVersionId} for metric spec ${spec.id}"
      )
    ...
    new MonitoringService[F] {
      override def create(spec: CustomModelMetricSpec): F[CustomModelMetricSpec] =
        for {
          ...
          deployedSpec =
            spec.copy(config = spec.config.copy(servable = monitorServable.started.some))
      ...
      override def delete(specId: String): F[CustomModelMetricSpec] =
        for {
          spec <- OptionT(repo.get(specId)).getOrElseF(
            F.raiseError(DomainError.NotFound(s"MetricSpec with id ${specId} not found"))
          )
          _ <- repo.delete(specId)
          _ <- spec.config.servable.traverse { servable =>
            logger.debug(
              s"Servable ${servable.fullName} is attached to MetricSpec ${specId}. Deleting..."
            )
