...
import io.hydrosphere.serving.manager.api._
...
class ManagerGrpcService[F[_]](
  ...
  override def getAllVersions(
      request: Empty,
      responseObserver: StreamObserver[grpc.entities.ModelVersion]
  ): Unit = {
  ...
  override def deployServable(
      ...
      responseObserver: StreamObserver[grpc.entities.Servable]
  ): Unit = {
    val flow = for {
      res <- request.modelVersion match {
        ...
        case ModelVersion.Fullname(value) =>
          servableService.findAndDeploy(value.name, value.version, request.metadata)
      }
  ...
  override def removeServable(request: RemoveServableRequest): Future[grpc.entities.Servable] = {
    val flow = for {
