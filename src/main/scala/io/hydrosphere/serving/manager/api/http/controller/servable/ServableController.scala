...
class ServableController[F[_]: Effect](
    ...
    path("servable" / Segment / "logs") { name =>
      get {
        parameter(Symbol("follow").as[Boolean].?) { follow: Option[Boolean] =>
          completeF {
            cloudDriver
              .getLogs(name, follow.getOrElse(false))
              .map { source =>
                source
    ...
    path("servable") {
      post {
        entity(as[DeployModelRequest]) { r =>
          completeF {
            servableService
              .findAndDeploy(r.modelName, r.version, r.metadata.getOrElse(Map.empty))
  ...
  val routes = listServables ~ getServable ~ deployModel ~ stopServable ~ sseServableLogs
}
