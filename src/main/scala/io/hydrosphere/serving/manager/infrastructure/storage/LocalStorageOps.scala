...
class LocalStorageOps[F[_]: Sync] extends StorageOps[F] {
  ...
  override def getReadableFile(path: Path): F[Option[File]] =
    Sync[F].delay {
  ...
  override def copyFile(src: Path, target: Path): F[Path] =
    Sync[F].delay {
  ...
  override def moveFolder(oldPath: Path, newPath: Path): F[Path] =
    Sync[F].delay {
  ...
  override def writeBytes(path: Path, bytes: Array[Byte]): F[Path] =
    Sync[F].delay {
