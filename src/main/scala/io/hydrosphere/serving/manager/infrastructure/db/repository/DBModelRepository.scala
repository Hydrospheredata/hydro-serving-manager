...
object DBModelRepository {
  ...
  def getByNameQ(name: String): doobie.Query0[ModelRow] =
    sql"SELECT * FROM hydro_serving.model WHERE name = $name".query[ModelRow]
  def getByIdQ(id: Long): doobie.Query0[ModelRow] =
    sql"SELECT * FROM hydro_serving.model WHERE model_id = $id".query[ModelRow]
  def createQ(m: ModelRow): doobie.Update0 =
    sql"INSERT INTO hydro_serving.model (name) VALUES (${m.name})".update
  def updateQ(m: ModelRow): doobie.Update0 =
    sql"UPDATE hydro_serving.model SET name = ${m.name} WHERE model_id = ${m.model_id}".update
  def deleteQ(id: Long): doobie.Update0 =
    sql"DELETE FROM hydro_serving.model WHERE model_id = $id".update

  def make[F[_]]()(implicit F: MonadCancel[F, Throwable], tx: Transactor[F]): ModelRepository[F] =
