...
import io.hydrosphere.serving.manager.domain.contract.DataType
...
object TypeMapper {
  ...
  val dtypesToNames: Map[DataType, String] =
    nameToDtypes.view.filterKeys(_ != "half").toMap.map(_.swap)

  def toType(dtype: String): Option[DataType] =
    nameToDtypes.get(dtype)
