...
object KubernetesClient extends UnsafeLogging {
  ...
  def apply[F[_]: Async](
    ...
    KubernetesClient[F](
      config,
      dockerRepoConf,
      k8sInit(K8SConfiguration.useProxyAt(s"http://${config.proxyHost}:${config.proxyPort}"))
        .usingNamespace(config.kubeNamespace)
    )
  ...
  )(implicit ex: ExecutionContext): KubernetesClient[F] =
    new KubernetesClient[F] {
      ...
      override def runDeployment(
        ...
        val image          = dockerImage.copy(user = Some(dockerRepoHost))
        ...
        val podTemplate = Pod.Template
          .Spec(
            metadata = ObjectMeta(name = servable.name),
            spec = Some(podSpec)
          )
          .addContainer(
            Container("model", image.fullName)
        ...
        for {
          ...
          maybeExist = deployed.find(
            _.metadata.labels.getOrElse(CloudDriver.Labels.ServiceName, "") == name
          )
      ...
      override def runService(name: String, servable: CloudInstance): F[skuber.Service] = {
        val service = skuber
          .Service(metadata = ObjectMeta(name = servable.name))
          .withSelector(CloudDriver.Labels.ServiceName -> name)
          .exposeOnPort(
            skuber.Service.Port("grpc", Protocol.TCP, DefaultConstants.DEFAULT_APP_PORT)
          )
          .addLabels(
            Map(
              CloudDriver.Labels.ServiceName    -> name,
      ...
      override def removeDeployment(name: String): F[Unit] =
        for {
          deployed <- deployments
          maybeDeployment = deployed.find(
            _.metadata.labels.getOrElse(CloudDriver.Labels.ServiceName, "") == name
          )
          _ <- maybeDeployment match {
            case Some(value) =>
              AsyncUtil.futureAsync(underlying.delete[Deployment](value.metadata.name))
            case None =>
              Async[F].delay(logger.error(s"kube deployment with name `$name` not found"))
          }
        } yield ()

      override def removeService(name: String): F[Unit] =
        for {
          deployed <- services
          maybeService = deployed.find(
            _.metadata.labels.getOrElse(CloudDriver.Labels.ServiceName, "") == name
          )
          _ <- maybeService match {
            case Some(value) =>
              AsyncUtil.futureAsync(underlying.delete[Service](value.metadata.name))
      ...
      override def getLogs(podName: String, follow: Boolean): F[Source[String, _]] =
        AsyncUtil
          .futureAsync(
            underlying.getPodLogSource(podName, Pod.LogQueryParams(follow = Some(follow)))
          )
      ...
      override def getPod(name: String): F[Pod] = {
        import LabelSelector.dsl._
        AsyncUtil
          .futureAsync(underlying.listSelected[PodList](CloudDriver.Labels.ServiceName is name))
          .flatMap { pods: PodList =>
            pods.toList match {
              case head :: _ => Async[F].pure(head)
              case Nil =>
                Async[F].raiseError(new RuntimeException(s"There is no running pods for $name"))
            }
          }
      }
    }
