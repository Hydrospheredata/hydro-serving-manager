...
class ECSImageRepository[F[_]: Sync](
  ...
  override def getImage(modelName: String, modelVersion: String): DockerImage =
    DockerImage(
      name =
        s"${ecsDockerRepositoryConfiguration.accountId}.dkr.ecr.${ecsDockerRepositoryConfiguration.region.getName}.amazonaws.com/$modelName",
      tag = modelVersion
    )

  private def getDockerRegistryAuth: F[DockerRegistryAuth] =
    Sync[F].blocking {
  ...
  private def createRepositoryIfNeeded(modelName: String): F[Unit] =
    Sync[F].blocking {
      ...
      try ecrClient.describeRepositories(req)
      catch {
