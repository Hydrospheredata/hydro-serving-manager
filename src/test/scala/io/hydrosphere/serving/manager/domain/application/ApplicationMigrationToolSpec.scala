...
import io.hydrosphere.serving.manager.infrastructure.db.repository.DBApplicationRepository.mapFromDb
...
class ApplicationMigrationToolSpec extends GenericUnitTest {
  ...
  describe("ApplicationMigrationTool") {
    it("should detect and recover invalid apps") {
      ...
      val data1 = Tables.ApplicationRow(1, "test", None, "Ready", "", graph, List.empty, List.empty, None)
      val data2 = Tables.ApplicationRow(1, "test", None, "Ready", "", graph, List.empty, List.empty, None)
      ...
      val appsRepo = new ApplicationRepository[IO] {
        ...
        override def all(): IO[List[GenericApplication]] = {
          IO.fromEither {
            List(data1, data2)
              .traverse(x => DBApplicationRepository.mapFromDb(x, Map.empty).toValidatedNec)
              .leftMap { errors =>
