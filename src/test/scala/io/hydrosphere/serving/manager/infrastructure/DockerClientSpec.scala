...
class DockerClientSpec extends GenericUnitTest {
  ...
  describe("DockerClient") {
    it("should handle logs correctly") {
      ...
      val conf = ContainerConfig.builder()
        .image("busybox")
      ...
      val logs = client.logs(containerName, true)
      println("logs")
      // println(logs.readFully())
      val fiber = logs.evalTap(x => IO(print(x))).compile.drain.start.unsafeRunSync()
      (IO.sleep(4.seconds) >> fiber.cancel >> client.removeContainer(containerName, List(RemoveContainerParam.forceKill()))).unsafeRunSync()
      succeed
    }
  }
}