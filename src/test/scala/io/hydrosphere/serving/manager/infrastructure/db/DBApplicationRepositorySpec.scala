...
class DBApplicationRepositorySpec extends GenericUnitTest {
  describe("DBApplicationRepo") {
    it("should raise error on incompatible application graph") {
      ...
      val data = ApplicationRow(1, "test", None, "Ready", "", graph, List.empty, List.empty, None)
      val res = DBApplicationRepository.mapFromDb(data, Map.empty)
